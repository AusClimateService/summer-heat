...
PROJECT_DIR=/g/data/xv83/dbi599/summer-heat
VAR=tas
UNITS=${VAR}='C'
...
IO_OPTIONS=--variables ${VAR} --spatial_coords -44 -11 113 154 --time_freq Q-NOV --time_agg ${TIME_AGG} --season ${SEASON} --shapefile ${SHAPEFILE} --spatial_agg ${SPATIAL_AGG} --units ${UNITS} --units_timing middle ${MODEL_IO_OPTIONS} --input_freq M
OBS_TXT_DATA=${PROJECT_DIR}/data/tas_BoM_1911-2022_${SEASON}_aus-mean.txt
OBS_NC_DATA=${PROJECT_DIR}/data/tas_BoM_1911-2022_${SEASON}_aus-mean.nc
OBS_BASELINE=27.5
OBS_URL=http://www.bom.gov.au/climate/change/
FCST_DATA=file_lists/${MODEL}_${EXPERIMENT}_files.txt
FCST_ENSEMBLE_FILE=${PROJECT_DIR}/data/${VAR}_${MODEL}-${EXPERIMENT}_${TIME_PERIOD_TEXT}_${SEASON}_aus-${SPATIAL_AGG}.zarr.zip
INDEPENDENCE_PLOT=${PROJECT_DIR}/figures/independence-test_${VAR}_${MODEL}-${EXPERIMENT}_${TIME_PERIOD_TEXT}_${SEASON}_aus-${SPATIAL_AGG}.png
FCST_BIAS_FILE=${PROJECT_DIR}/data/${VAR}_${MODEL}-${EXPERIMENT}_${TIME_PERIOD_TEXT}_${SEASON}_aus-${SPATIAL_AGG}_bias-corrected-BoM-${BIAS_METHOD}.zarr.zip
SIMILARITY_BIAS_FILE=${PROJECT_DIR}/data/ks-test_${VAR}_${MODEL}-${EXPERIMENT}_${BASE_PERIOD_TEXT}_${SEASON}_aus-${SPATIAL_AGG}_bias-corrected-BoM-${BIAS_METHOD}.zarr.zip
SIMILARITY_RAW_FILE=${PROJECT_DIR}/data/ks-test_${VAR}_${MODEL}-${EXPERIMENT}_${BASE_PERIOD_TEXT}_${SEASON}_aus-${SPATIAL_AGG}_BoM.zarr.zip


## format-obs : txt to nc
format-obs : ${OBS_NC_DATA}
${OBS_NC_DATA} : ${OBS_TXT_DATA}
	python bom_txt_to_nc.py $< ${OBS_BASELINE} ${OBS_URL} $@ 
